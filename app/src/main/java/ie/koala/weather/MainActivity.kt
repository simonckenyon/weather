...
import ie.koala.weather.api.GetWeatherService
...
class MainActivity : AppCompatActivity(), LocationListener {
    ...
    private fun getCurrentWeather(lat: String, lon: String) {
        ...
        doAsync {
            val service = RetrofitInstance.retrofitInstance.create(GetWeatherService::class.java)
            val call: Call<Today> = service.getToday(key, lat, lon, UNITS)
            call.enqueue(object : Callback<Today> {
                override fun onResponse(call: Call<Today>, response: Response<Today>) {
    ...
    private fun getFiveDayForecast(lat: String, lon: String) {
        ...
        doAsync {
            val service = RetrofitInstance.retrofitInstance.create(GetWeatherService::class.java)
            val call: Call<Forecast> = service.getForecast(key, lat, lon, UNITS, NUMBER_OF_DAYS)
                ...
                override fun onResponse(call: Call<Forecast>, response: Response<Forecast>) {
                    ...
                    if (forecast != null) {
                        try {
                            ...
                            uiThread {
                                val testAdapter = ForecastAdapter(this@MainActivity, 0, forecastItemList)
